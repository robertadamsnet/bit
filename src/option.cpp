...
//============================================================================
//
auto parse(const list_t& opts, const action_t& non_opt_action, 
    const default_t& default_action) 
  -> std::function<int(int, char*[])> 
{
  return [&](int argc, char* argv[]) -> int {
    ...
    list_t opt_list(opts.begin(), opts.end());
    list<item_t> l(opt_list.begin(), opt_list.end());
    ...
    static auto builtin_help_opt = item_t('h', "help", 
    ...
    while(optind < argc) {
      auto r = non_opt_action(argv[optind++]);
      if(r) {
        return r;
      }
    }    
